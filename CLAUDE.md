...
- **Typography resolves per platform and its values are not provisional**: as of 0.10.0 `CruxTypography`'s nine semantic tokens resolve to Apple's HIG scale on iOS/macOS and to Material 3 elsewhere, so a token has different metrics on different platforms by design (see `unknowns/typography-hig/ledger.md`). Never patch a size or weight onto a token at a call site — add or pick a token instead; the two `copyWith(fontSize:)` habits that existed before 0.10.0 are exactly what this replaced. Reshaping a token is done by passing that token's override to the `CruxTypography` a `CruxThemeData` is built from — that is the sanctioned customisation path as of 0.11.0, and it does not loosen the ban on patching a token at a call site. `test/tokens/native_scale_parity_test.dart` checks both tiers against their upstream sources (the `cupertino_typography` dev dependency, and Flutter's own `Typography.englishLike2021`), so a drifting transcription fails CI.
...
- **Catalog operating rule**: `widgetbook/` (repo root, sibling to `example/`) is the dev catalog app — `widgetbook: ^3.25.0` is a dependency of that app only, never of the package itself. **Every new component must land with a widgetbook use-case set (Playground / States matrix / Edge cases, in `widgetbook/lib/usecases/<name>.dart`) plus a golden test pair (light + dark) in `widgetbook/test/golden_test.dart`**, following the contract in `widgetbook/lib/usecases/CONVENTIONS.md`. Exhaustive variant coverage is widgetbook's job alone, and only widgetbook's — that doesn't change. `example/` is one mock app, "ミモザとの暮らし" (life with Mimosa, an AI companion), not a gallery of demo screens: a login screen leads into a 4-tab shell (home/tasks, Mimosa/chat, household ledger, settings) plus a "きろく" (journal entry) post modal opened from home's FAB — see `example/lib/main.dart` for the app shell and `example/lib/screens/` for each screen. When a new component lands, find where it fits as a natural part of that existing flow (a settings row, a list item, a dialog trigger, and so on) and add it there. Only add a new screen when no existing flow has a natural home for it, treating the addition as a real feature of the app rather than a demo slot. `example/` still never grows a token table or a per-variant state grid — those stay in widgetbook.
