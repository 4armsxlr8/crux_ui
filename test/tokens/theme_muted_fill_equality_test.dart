// Equality/hashCode regression for CruxColors.mutedFill.
//
// This lives in its own file rather than inside crux_theme_test.dart's
// existing "CruxThemeData equality" group, which already has equivalent
// tests for colors.controlFill and (in theme_control_plate_equality_test.dart)
// colors.controlPlate: crux_theme_test.dart is out of scope for the change
// that fixes mutedFill's omission, so this mirrors that same test's shape
...
void main() {
  group('CruxThemeData equality', () {
    test('two instances differing only in colors.mutedFill are not equal '
      ...
      final CruxThemeData b = CruxThemeData(
        colors: CruxColors(
          ...
          onAccent: CruxColors.light.onAccent,
          mutedFill: const Color(0xFF000000),
          controlPlate: CruxColors.light.controlPlate,
        ),
        typography: const CruxTypography(),
        brightness: Brightness.light,
      );
